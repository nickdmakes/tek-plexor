...
def add_tags(file, dp_params: dict, md_params: dict):
    format = dp_params[dp.COMPRESSION]
    if format == dp.M4A or format == dp.MP4:
...
def convert(file: str, dp_params: dict, md_params: dict):
    conv_file = Path(file).with_suffix("." + dp_params[dp.COMPRESSION])
    
    if os.path.exists(conv_file):
        raise FileExistsException(f"File already exists", conv_file)
    
    encoder = EncoderMappings(dp_params[dp.COMPRESSION]).get_encoder()
    bitrate = str(dp_params[dp.BITRATE]) + "k"
    try:
        ...
        ffmpeg.execute()
        if dp_params[dp.DELETE_OG]:
            os.remove(file)
    except FFmpegError as e:
        raise AudioConversionException(e)
    
    if dp_params[dp.ADD_TAGS]:
        add_tags(conv_file, dp_params, md_params)
